...
function TransportChoices({ transport, onChoose }: { readonly transport: "encrypted" | "standard"; readonly onChoose: (transport: "encrypted" | "standard") => void }) {
  return <RadioGroup className="upstream-transport-choices" aria-label="Connection privacy" value={transport} onValueChange={onChoose}>
    <RadioCard value="encrypted"><LockKeyhole size={17} /><span><strong>Encrypted</strong><small>Recommended · HTTPS</small></span></RadioCard>
    <RadioCard value="standard"><Network size={17} /><span><strong>Standard DNS</strong><small>Maximum compatibility</small></span></RadioCard>
  </RadioGroup>;
}
...
function ResolverProfileButton({ profile, probes, selectedSet, fastestProfileID, transport, catalogLoaded, encryptedByAddress, probing, onToggle }: { readonly profile: ResolverProfile; readonly probes: Record<string, UpstreamProbe>; readonly selectedSet: Set<string>; readonly fastestProfileID: string | null; readonly transport: "encrypted" | "standard"; readonly catalogLoaded: boolean; readonly encryptedByAddress: Map<string, EncryptedUpstreamEndpoint>; readonly probing: boolean; readonly onToggle: (profile: ResolverProfile) => void }) {
  ...
  return <button className={`resolver-profile ${fullySelected ? "selected" : ""} ${partiallySelected ? "partial" : ""} ${encryptedUnavailable ? "unavailable" : ""}`} type="button" onClick={() => onToggle(profile)} aria-pressed={partiallySelected ? "mixed" : fullySelected} disabled={encryptedUnavailable}>{(fullySelected || partiallySelected) && <span className="profile-check" aria-hidden="true">{fullySelected ? <Check size={16} /> : selectedCount}</span>}<span className="profile-copy"><span className="profile-title-row"><strong>{profile.name}</strong></span><span className="profile-title-badges">{profile.id === fastestProfileID && <em className="fastest">Fastest</em>}{profile.recommended && <em>Recommended</em>}{transport === "encrypted" && encryptedEndpoint && <em className="encrypted"><LockKeyhole size={11} /> HTTPS available</em>}{transport === "encrypted" && !catalogLoaded && <em>Checking HTTPS</em>}{encryptedUnavailable && <em className="standard-only">Standard only</em>}</span><span>{profile.description}</span>{transport === "encrypted" && <span className="profile-latency"><Gauge size={14} /><ProbeText probe={profileProbe} loading={probing && !profileProbe} /></span>}<ProfileEndpoint profile={profile} transport={transport} endpoint={encryptedEndpoint} endpointText={endpointText} probes={probes} probing={probing} /></span><span className="profile-badges">{profile.badges.map((badge) => <span key={badge}>{badge}</span>)}</span></button>;
}
...
function UpstreamSelectionPanel({ selectionCount, selectionLabel, dirty, transport, mixesFiltering, selected, selectedSet, selectedEncryptedEndpoints, unsupportedEncrypted, probes, probing, catalogLoaded, catalogError, saveState, message, configured, configuredTransport, setSelected, setTransport, setMessage, setSaveState, save }: { readonly selectionCount: number; readonly selectionLabel: string; readonly dirty: boolean; readonly transport: "encrypted" | "standard"; readonly mixesFiltering: boolean; readonly selected: string[]; readonly selectedSet: Set<string>; readonly selectedEncryptedEndpoints: EncryptedUpstreamEndpoint[]; readonly unsupportedEncrypted: string[]; readonly probes: Record<string, UpstreamProbe>; readonly probing: boolean; readonly catalogLoaded: boolean; readonly catalogError: string; readonly saveState: "idle" | "saving" | "saved" | "error"; readonly message: string; readonly configured: string[]; readonly configuredTransport: "encrypted" | "standard"; readonly setSelected: (value: string[]) => void; readonly setTransport: (value: "encrypted" | "standard") => void; readonly setMessage: (value: string) => void; readonly setSaveState: (value: "idle" | "saving" | "saved" | "error") => void; readonly save: () => Promise<void> }) {
  return <aside className="upstream-selection-panel panel" aria-label="Selected upstreams"><div className="selection-heading"><div><span>Current selection</span><strong>{selectionCount} {selectionLabel}</strong></div><span className={`selection-status ${dirty ? "pending" : ""}`}>{dirty ? <AlertTriangle size={15} /> : <ShieldCheck size={15} />} {dirty ? "Unsaved" : "Active"}</span></div><SelectionPrivacyState transport={transport} />{mixesFiltering && <div className="selection-policy-note"><AlertTriangle size={16} /><div><strong>Mixed filtering</strong><span>Filtered and unfiltered resolvers are selected, so blocking results can vary.</span></div></div>}<SelectedServerList selected={selected} selectedSet={selectedSet} transport={transport} selectedEncryptedEndpoints={selectedEncryptedEndpoints} unsupportedEncrypted={unsupportedEncrypted} probes={probes} probing={probing} catalogLoaded={catalogLoaded} setSelected={setSelected} /><div className="selection-note"><strong>{transport === "encrypted" ? "Private by design" : "How latency is measured"}</strong><span>{selectionNote(transport)}</span></div><div className="selection-actions"><Button variant="outline" type="button" className="secondary" disabled={!dirty} onClick={() => { setSelected(configured); setTransport(configuredTransport); setMessage(""); setSaveState("idle"); }}><RotateCcw size={16} /><span>Reset</span></Button><Button variant="default" type="button" disabled={!dirty || saveState === "saving" || selected.length === 0 || (transport === "encrypted" && (!catalogLoaded || Boolean(catalogError) || unsupportedEncrypted.length > 0))} onClick={() => void save()}><Save size={16} /><span>{saveState === "saving" ? "Saving" : "Save upstreams"}</span></Button></div>{message && <span role={saveState === "error" ? "alert" : "status"} className={`selection-message ${saveState === "error" ? "error" : ""}`}>{message}</span>}</aside>;
}
...
function SelectedUnsupportedServer({ server, selected, probe, probing, setSelected }: { readonly server: string; readonly selected: string[]; readonly probe?: UpstreamProbe; readonly probing: boolean; readonly setSelected: (value: string[]) => void }) {
  ...
  return <div className="selected-server unsupported-endpoint"><span className="selected-provider-logo">{match ? <ProviderLogo providerID={match.provider.id} providerName={match.provider.name} /> : <Server size={15} />}</span><div><strong>{server}</strong><span>No supported HTTPS endpoint</span></div><ProbeBadge probe={probe} loading={probing && !probe} compact /><Button variant="ghost" size="icon" className="icon-button" type="button" onClick={() => setSelected(selected.filter((address) => address !== server))} aria-label={`Remove ${server}`}><X size={15} /></Button></div>;
}
...
function SelectedStandardServer({ server, selected, probe, probing, setSelected }: { readonly server: string; readonly selected: string[]; readonly probe?: UpstreamProbe; readonly probing: boolean; readonly setSelected: (value: string[]) => void }) {
  const match = findUpstreamAddress(server);
  return <div className="selected-server"><span className="selected-provider-logo">{match ? <ProviderLogo providerID={match.provider.id} providerName={match.provider.name} /> : <Server size={15} />}</span><div><strong>{server}</strong><span>{match ? `${match.provider.name} · ${match.profile.name}` : "Custom resolver"}</span></div><ProbeBadge probe={probe} loading={probing && !probe} compact /><Button variant="ghost" size="icon" className="icon-button" type="button" onClick={() => setSelected(selected.filter((address) => address !== server))} aria-label={`Remove ${server}`}><X size={15} /></Button></div>;
}
